    ...
    extends WizardPage
{
    ...
    private List<RequireProjectGroup> input =
        new ArrayList<RequireProjectGroup>();
    ...
    private final class ProjectsContentProvider
    ...
    private final class ProjectsComparator
    ...
    private final class ProjectsLabelProvider
    ...
    public List<RequireProjectGroup> getProjects()
    {
        ...
        for (TreeRequireElement ee : input)
        {
            ...
            {
                List<RequireProjectEntry> entries =
                    ((RequireProjectGroup)ee).getEntries();
                List<RequireProjectEntry> enabled =
                    new ArrayList<RequireProjectEntry>();
    ...
    public static class RequireProjectEntry
        extends TreeRequireElement
    {
    ...
    public static class RequireProjectGroup
        extends TreeRequireElement
    {
    ...
    private ICheckStateProvider checkStateProvider = new ICheckStateProvider()
    {
        ...
        public boolean isGrayed(Object element)
        {
            ...
            {
                List<RequireProjectEntry> entries =
                    ((RequireProjectGroup)element).getEntries();
                boolean a = false;
                boolean b = false;
    ...
    private void createViewer(Composite control, int span)
    {
        ...
        GridDataFactory.fillDefaults().grab(true, true).span(span, 1).hint(600,
            300).applyTo(tree.getControl());
    ...
    public void setPreviousPage(IWizardPage page)
    {
        ...
        try
        {
            final List<IRequireImportLocation> selection =
                ((RequireLocationPage)page).getSelection();
        ...
        IProgressMonitor monitor)
    {
        ...
        for (IRequireImportLocation loc : selection)
        {
            ...
            List<RequireProject> list =
                RequireProjectEngine.findDotProjects(loc.getLocation(), sub);
    ...
    private void groupProjects(List<RequireProjectGroup> workingSets)
    {
        ...
        Map<IPath, List<RequireProjectEntry>> prefixes =
            new HashMap<IPath, List<RequireProjectEntry>>();
        for (RequireProjectGroup group : workingSets)
        {
            for (RequireProjectEntry prj : group.getEntries())
            {
                ...
                IPath path =
                    new Path(prj.getLocation().getName()).append(prjPath.removeLastSegments(1));
                if (prjPath.lastSegment().startsWith("com._1c.g5.v8"))
                {
                    path =
                        path.append(prjPath.lastSegment().substring(13).replace(
                            ".", "/"));
                }
        ...
        loop: while (true)
        {
            ...
            for (IPath path : keys)
            {
                ...
                if (list.size() == 1)
                {
                    ...
                    List<RequireProjectEntry> parent =
                        prefixes.get(path.removeLastSegments(1));
                    if (parent != null)
                    {
                        parent.addAll(list);
        ...
        tree.getControl().getDisplay().asyncExec(new Runnable()
        {
            ...
            public void run()
            {
                final String includePattern =
                    (String)includeFilterValue.getValue();
                final String excludePattern =
                    (String)excludeFilterValue.getValue();
                for (TreeRequireElement ee : input)
                {
                    if (ee instanceof RequireProjectGroup)
                    {
